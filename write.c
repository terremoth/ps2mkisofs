...
void FDECL1(psx2_sort, struct directory *, node)
{
  struct directory_entry * second_entry; 
  struct directory_entry * depnt;
  struct directory_entry * sys_conf;
  int done;

  /* shift system.cnf to the top */

  second_entry = node->contents->next; /* (skip past .) */
  depnt = second_entry->next;
...
/*    fprintf(stderr, "scanning %s\n", depnt->name); */
    if (depnt->next)
    {
      if (!strcmp(depnt->next->name, "system.cnf"))
      {
        struct directory_entry * temp;

        sys_conf = depnt->next;
        temp = depnt->next;
        depnt->next = depnt->next->next; /* Take entry out of loop */
        temp->next = second_entry->next;
        second_entry->next = temp;
      }
    }
    depnt = depnt->next;
  }  
  ...
  while (!done)
  {
    ...
    while (depnt)
    {
      if (depnt->next)
      {
        if ((depnt->next->size / SECTOR_SIZE + sectors_eaten) < (12231 - 255))
        {
          struct directory_entry * temp;

          sectors_eaten += depnt->next->size / SECTOR_SIZE;
...
/*		  fprintf(stderr,"Size %d ", dwpnt->size); */
	  ...
	  {
               if (strcmp(dwpnt->name + strlen(dwpnt->name) - 10, "system.cnf") == 0)
               {
                 int padding = sys_conf_pad;
                 while (padding > 0)
                 {
                   xfwrite("This here is padding yo! ", 1, padding > 25 ? 25 : padding ,outfile);
                   padding -= 25;
                 }
               }
#ifdef VMS
		...
		*/
	       if(s_entry->size) 
	       {
		    ...
		    dwpnt->size = s_entry->size;
                    if ((strcmp(s_entry->name, "system.cnf") == 0) && (last_extent < 12231))
                    {
                      sys_conf_pad = (12231 - last_extent) * SECTOR_SIZE;
                      last_extent = 12231;
                    }
